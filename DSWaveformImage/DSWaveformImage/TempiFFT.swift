    ...
    public func fftForward(_ inMonoBuffer:[Float]) {
        ...
        if self.windowType != .none {
            ...
            if self.window == nil {
                ...
                let x = UnsafeMutablePointer(mutating: self.window!)
                switch self.windowType {
                case .hamming:
                    vDSP_hamm_window(x, UInt(size), 0)
                case .hanning:
                    vDSP_hann_window(x, UInt(size), Int32(vDSP_HANN_NORM))
                default:
                    break
                }
            }
...
//        let iFloats = [Float](UnsafeBufferPointer(start: self.complexBuffer.imagp, count: self.halfSize))
        ...
        let x = UnsafeMutablePointer(mutating: self.magnitudes!)
        vDSP_zvmags(&(self.complexBuffer!), 1, x, 1, UInt(self.halfSize))
